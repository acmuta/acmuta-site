...
const EventCard: React.FC<EventCardProps> = ({ eventName, image1, image2, isSingle }) => {
  return (
    <div
      className={`bg-white shadow-lg rounded-lg p-6 text-center border border-gray-200 flex flex-col items-center 
      ${isSingle ? "w-full md:w-3/4 lg:w-2/3 xl:w-1/2 mx-auto" : "w-full"}`}
    >
      <h2 className="text-2xl font-semibold mb-4 text-gray-800">{eventName}</h2>
